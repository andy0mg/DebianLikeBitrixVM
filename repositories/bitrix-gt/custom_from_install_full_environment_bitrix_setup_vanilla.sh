...
#	echo 'AstraMode off' >> /etc/apache2/apache2.conf	
  [ -d /etc/systemd/system/apache2.service.d ] || mkdir /etc/systemd/system/apache2.service.d
...
dplMYSQL() {
	...
	mysql -e "create database IF NOT EXISTS bitrix;create user IF NOT EXISTS bitrix@localhost;grant all on bitrix.* to bitrix@localhost;set password for bitrix@localhost = PASSWORD('${DBPASS}')"
